...
public enum GameMode {Random, Custom, Play, ParamTest};
...
//	private bool fin_d = true;
	...
	[DllImport("__Internal")]
	private static extern void ParamTestEvent (int index);
	private int test_event_counter = 15;
	private int sampling_rate = 15;

	[DllImport("__Internal")]
	private static extern void ParamTestLoadNextLevel ();
	...
	void start_game (){
		...
		if (game_mode != GameMode.ParamTest) {
			end_position = 30;
		}
		last_df = 0;
		last_sd = 0;
		...
		if (game_mode == GameMode.ParamTest) {
			Debug.Log ("game_mode is ParamTest "+test_event_counter+"/"+sampling_rate);
			// TODO: parameter verändern
			if (test_event_counter >= sampling_rate) {
				test_event_counter = 0;
				ParamTestLoadNextLevel ();
				ParamTestEvent (test_event_counter++);
			} else {
				ParamTestEvent (test_event_counter++);
			}
		}
...
//			Destroy (player[i]);
	...
	void FixedUpdate() {
		...
		if (player.Length <= 0 && dead.Length > 0 && player.Length + dead.Length == number_players) {
			#if !UNITY_EDITOR && UNITY_WEBGL
				if((game_mode == GameMode.Random || game_mode == GameMode.ParamTest) && game_mode == next_game_mode)
					SaveParameters ();
	...
	void testmode() {
		next_game_mode = GameMode.ParamTest;
		test_event_counter = sampling_rate;
		#if !UNITY_EDITOR && UNITY_WEBGL
			WebGLInput.captureAllKeyboardInput = false;
		#endif
	}

	void set_endposition(double pos) { 
		end_position = pos;
		Debug.Log ("New end position: " + pos);
	}
