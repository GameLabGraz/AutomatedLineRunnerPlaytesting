...
mergeInto(LibraryManager.library, {
  ...
  ParamTestEvent: function(index) {
      current_val = change_test_parameter(index)
  },

  ParamTestLoadNextLevel: function() {
      if (level_index < levels.length) {
          // console.log("import level", JSON.parse(levels[level_index]));
          var s = JSON.stringify(levels[level_index++]);
          importLevelString(s)
      }else{
          pause();
          alert("all levels finished. Results saved in level_data variable")
      }
  },
  ...
  SaveParameters: function() {
      ...
      // paramtest begin
      if (current_val && level_data.length > level_index-1) {
          data["value"] = current_val
          level_data[level_index-1].push(data)
      }
      // paramtest end
